...
function AutoLayout() {
  const BpmnModdleCtr = BpmnModdle.default || BpmnModdle;
  this.moddle = new BpmnModdleCtr();
...
AutoLayout.prototype._layoutConnections = function(group, parentDi) {
  ...
  connections.forEach(function(connection) {
    ...
    if (sourceNode && sourceNode.children && sourceNode.children.length > 0 && !directTarget && connectionDi.waypoint && connectionDi.waypoint.length > 0) {
...
function getEndEvents(flowElements) {
  ...
  const endEvents = flowElements.filter(e => {
    return is(e.$type, 'bpmn:EndEvent');
  });

  return (endEvents.length > 0) ? endEvents : flowElements.filter(e => {
    const incoming = e.get('incoming');
    const outgoing = e.get('outgoing');
    return (incoming && incoming.length > 0) && (outgoing || (outgoing && outgoing.length === 0));
  });
}
