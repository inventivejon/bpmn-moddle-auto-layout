...
Tree._apportion = function(tree, node, level) {
  ...
  for (var k = tree.config.iMaxDepth - level; firstChild != null && firstChildLeftNeighbor != null && j <= k;) {
    ...
    rightAncestor = rightAncestor && rightAncestor.parents[0];
    var modifierSumRight = (rightAncestor && rightAncestor.modifier) || 0;

    leftAncestor = leftAncestor && leftAncestor.parents[0];
    var modifierSumLeft = (leftAncestor && leftAncestor.modifier) || 0;
    ...
    if (totalGap > 0) {
      ...
      var numSubtrees = 0;
      const visited = [];
      for (; subtreeAux != null && subtreeAux !== leftAncestor; subtreeAux = subtreeAux && subtreeAux._getLeftSibling()) {
        numSubtrees++;
        const found = visited.find(sibling => sibling === subtreeAux);
        if (found) {
          subtreeAux = null;
        } else {
          visited.push(subtreeAux);
        }
      }
      if (subtreeAux != null) {
